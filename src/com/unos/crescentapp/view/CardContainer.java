...
public class CardContainer extends AdapterView<ListAdapter>{
    ...
    private OnAdapterEmptyListener mOnAdapterEmptyListener; 
    private OnCardDismissedListener mOnCardDisListener;
	public OnAdapterEmptyListener getmOnAdapterEmptyListener() {
		return mOnAdapterEmptyListener;
	}

	public void setmOnAdapterEmptyListener(
			OnAdapterEmptyListener mOnAdapterEmptyListener) {
		this.mOnAdapterEmptyListener = mOnAdapterEmptyListener;
	}
	public OnCardDismissedListener getmOnCardDisListener() {
		return mOnCardDisListener;
	}

	public void setmOnCardDisListener(OnCardDismissedListener mOnCardDisListener) {
		this.mOnCardDisListener = mOnCardDisListener;
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		switch (event.getActionMasked()) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				dx = x - mLastTouchX;
				dy = y - mLastTouchY;
				likeImage = mTopCard.findViewById(R.id.adapter_discovery_profile_like);
				dislikeImage = mTopCard.findViewById(R.id.adapter_discovery_profile_dislike);
			...
			case MotionEvent.ACTION_CANCEL:
				...
				animator.addListener(new AnimatorListener() {
					...
					public void onAnimationEnd(Animator animation) {
						...
						//((FindingPeople) getAdapter().getItem(0)).getmOnCardDimissedListener().cancelCard();
					}
	...
	public boolean onInterceptTouchEvent(MotionEvent event) {
		...
		switch (event.getActionMasked()) {
			case MotionEvent.ACTION_DOWN:
				...
				FindingPeople findingPeople = (FindingPeople)getAdapter().getItem(0);

                if (findingPeople.getmOnClickListener() != null) {
                	findingPeople.getmOnClickListener().OnClickListener();
                }
				pointerIndex = event.getActionIndex();
				x = event.getX(pointerIndex);
				y = event.getY(pointerIndex);
	...
	private class GestureListener extends SimpleOnGestureListener {
		...
		public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
			...
			if (Math.abs(dx) > mTouchSlop &&
                ...
                if (mOnCardDisListener != null) {
                    if ( targetX > 0 ) {
                    	mOnCardDisListener.onLike((FindingPeople)topCard.getTag());
                    	
                    	if(getChildCount()==1){
                    		mOnAdapterEmptyListener.onAdapterEmpty();
                    	}
                    } else {
                    	mOnCardDisListener.onDislike((FindingPeople)topCard.getTag());
                    	if(getChildCount()==1){
                    		mOnAdapterEmptyListener.onAdapterEmpty();
                    	}
                    }
